...
from app.api.security import SlidingWindowRateLimiter, score_rate_limiter
...
def test_rate_limit_returns_429(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "app.api.security.score_rate_limiter",
        SlidingWindowRateLimiter(max_events=2, window_seconds=300.0),
    )
    for i in range(2):
        r = client.post("/score", json={**_MINIMAL, "subject": f"m{i}"})
        assert r.status_code == 200, r.text
    r = client.post("/score", json={**_MINIMAL, "subject": "blocked"})
    assert r.status_code == 429
    monkeypatch.setattr("app.api.security.score_rate_limiter", score_rate_limiter)
...
def test_production_duplicate_request_id_returns_409(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    sig = _sign(raw, secret)
    headers = {"Content-Type": "application/json", HMAC_SIGNATURE_HEADER: sig}
    assert client.post("/score", content=raw, headers=headers).status_code == 200
    dup = client.post("/score", content=raw, headers=headers)
